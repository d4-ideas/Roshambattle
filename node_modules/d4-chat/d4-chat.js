...
exports.addPlus = function(plus, callback){
    if (typeof plus != 'undefined' &&
        ...
        var query = modChat.findOneAndUpdate({_id: plus.chatID, user:{$ne:plus.userID}, pluses: {$nin:[plus.userID]}}, { $push: {pluses: {user: plus.userID, plusDate: new Date()}} }, {new:true}, function (err, data){
            if (err)
                return callback({error: err}, undefined);
            else{
                if(data)
...
exports.addMinus = function(minus, callback){
    if (typeof minus != 'undefined' &&
        ...
        var query = modChat.findOneAndUpdate({_id: minus.chatID, user:{$ne:minus.userID}, minuses: {$nin:[minus.userID]}}, { $push: {minuses: {user: minus.userID, minusDate: new Date()}} }, {new:true}, function (err, data){
            if (err)
                return callback({error: err}, undefined);
            else{
                if(data)

...
exports.logBattle = function(user, callback){
    if(user){
        var battleupdate = {$unset:{'weapon':''}, $inc:{'totalBattles':1}};
        switch(user.result){
            case 'Win':
                battleupdate.$inc.totalWins = 1;
                break;
            case 'Tie': 
                battleupdate.$inc.totalTies = 1;
                break;
            case 'Loss':
                battleupdate.$inc.totalLosses = 1;
                break;
        } 
        var query = roshamuserModel.findOneAndUpdate({'userid':user.userid}, battleupdate,   function(err, foundUser){
//        query.where('userid').equals(user.userid);
//        query.findOneAndUpdate({$set:{'weapon':undefined}});
//        query.findOneAndUpdate({$inc:{'totalBattles':1}});
//        query.findOneAndUpdate(function(err, foundUser){
            if(err){
                return callback({'error': err}, undefined);
            }
            else{

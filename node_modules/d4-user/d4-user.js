...
// arguments    : user object {_id or email or tokenKey}
...
exports.register = function(newUser, callback){
    if(newUser){
        ...
        var query = userModel.findOne({'email': oUser.email});
...
// arguments    : User object {userID, name, email, mobile, password, token}
...
exports.update = function(user, callback){
    ...
    if (user.token)
        upUser.token = user.token;

    if(typeof user != 'undefined' && user.userID){
        userModel.findOneAndUpdate({_id:user.userID}, upUser, {new:true}, function(err, data){
            if (err)
        ...
        callback ({error:'You must supply the userID'}, undefined);
};
...
exports.verifyPassword = function(user, callback){
    if (user.userID && user.password){
        userModel.findOne({_id: user.userID, password: user.password}, function(err, data){
            if (err){
                callback({error:err}, undefined);
            }
            else {
        ...
        callback ({error: 'You must supply a userID and password'}, undefined);
    };
};

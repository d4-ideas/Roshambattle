...
//************************************************************************************************************
function getTurnResults(turnQuery, callback){
...
exports.getTurnResults = getTurnResults;
...
exports.getUserTurnResults = function(turnQuery, callback){
    getTurnResults(turnQuery, function(err, foundResults){
        ...
        if((typeof foundResults === 'object')&&(foundResults.length > 0)){
            var userResult = {'turnDate': turnQuery.turnDate,
                              'userID': turnQuery.userID};
            
            console.log('A');
            console.log('AAA' + foundResults[0].player1._id + 'XXX');
            console.log('ABA' + turnQuery.userID + 'XXX');
            if(foundResults[0].player1._id === turnQuery.userID){
                console.log('g');
                userResult.weapon = foundResults[0].weapon1;
            }
            else{
                console.log('h');
                userResult.weapon = foundResults[0].weapon2;
            }
            userResult.opponents = new Array(0);
            var newOpponent = {'name':'','result':''};

            for(var i=0;i<foundResults.length;i++){
                if(foundResults[i].winner === 0){
                    newOpponent.result = 'Tie';
                }
                if(foundResults[i].player1._id === turnQuery.userID){
                    newOpponent.name = foundResults[i].player2.name;
                    if(foundResults[i].winner === 1){
                    ...
                    newOpponent.name = foundResults[i].player1.name;
                    if(foundResults[i].winner === 2){
                        newOpponent.result = 'Win';
                    }
                    else{
                        newOpponent.result = 'Loss';
                ...
                userResult.opponents.push(newOpponent);
            }          
            console.log('CCC' + JSON.stringify(userResult));
            
            return callback(undefined, userResult);
        }
        else{
